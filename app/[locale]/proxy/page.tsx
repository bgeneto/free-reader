...
export async function generateMetadata({
  ...
  const fallbackMetadata: Metadata = {
    title: `${siteName} - Article Reader & Summarizer`,
  ...
  try {
    ...
    siteHostName = urlObj.hostname.replace('www.', '');
  ...
  if (article) {
    ...
    siteHostName = article.siteName || siteHostName;
  }

  const description = (article && article.textContent)
    ? article.textContent.slice(0, 160).trim() + "..."
    : `Read "${title}" from ${siteHostName} on ${siteName} - No paywalls, AI summaries available`;
      ...
      images: [
        {
          ...
          width: 1200,
          height: 630,
          alt: `${siteName} - AI Summarizer and Free Paywall Remover`,
        },
      ],
    },
    twitter: {
