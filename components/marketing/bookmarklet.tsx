...
export const BookmarkletLink = () => {
  ...
  // Simple bookmarklet - opens current page in proxy
  const siteUrl = process.env.NEXT_PUBLIC_URL || "https://smry.ai";
  const siteName = process.env.NEXT_PUBLIC_SITE_NAME || "SMRY";
  const bookmarklet = `javascript:void(function(){var url=window.location.href;window.open('${siteUrl}/proxy?url='+encodeURIComponent(url),'_blank');}());`;
